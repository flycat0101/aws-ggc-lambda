...
class NodeLookup(object):
	...
	def __init__(self,
		...
		if not label_lookup_path:
			label_lookup_path = model_path + '/' + 'imagenet_2012_challenge_label_map_proto.pbtxt'
		if not uid_lookup_path:
			uid_lookup_path = model_path + '/' + 'imagenet_synset_to_human_label_map.txt'
...
def run_inference_on_image(image):
	...
	with tf.Session() as sess:
		...
		for node_id in top_k:
			...
			info = '%s (score = %.5f)\n' % (human_string, score)
			inference_info = inference_info + info
		print(inference_info)

def greengrass_object_classification_run():
	image = model_path + '/' + image_name
...
def function_handler(event, context):
	...
	client.publish(topic = '/topic/inference', payload = inference_info)
